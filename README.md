...
- 文件备份 — 转换前自动备份原始文件,文件名追加 `.bak.`
- 灵活配置 — 支持自定义忽略目录和文件扩展名

## 安装

打开 VS Code,按 `Ctrl + P`,输入以下命令 `ext install buuug7.GBK2UTF8` 安装或者在扩展市场中搜索 **gbk**,**utf8**,**gbk2utf8** 等关键字搜索并安装.
...
按 `Ctrl + Shift + P` 打开命令面板,执行 `GBK2UTF8: Convert Encoding To UTF8` 命令.
...
在文件资源管理器中选中多个文件或文件夹,右键选择 **Convert Encoding To UTF8** 即可批量转换.

> 注意:同时选中多个文件夹时,仅处理第一个文件夹中的文件.

批量转换完成后,会在工作区根目录自动生成报告文件 `GBK2UTF8-result-2026-06-09T15-30-22.md`

### 文件备份

每次转换前,原始文件会在同级目录自动备份 `原始文件.txt → 原始文件.txt.bak`
